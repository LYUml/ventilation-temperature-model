...
The previously reported MZ-5R1C mean RMSE of 0.852 °C used archetype load magnitudes. Replacing those with real RDF hourly schedules produced 0.988 °C. Applying the construction-over-template RDF precedence for SHGC then produced 0.738 °C in the original RDF-schedule corridor scenario. That score is retained for provenance, but is not the score of the new 4F412 zero-internal-gain validation scenario. All scenarios remain non-compliant because the remaining solar, thermal-mass and HVAC/airflow boundary issues are unresolved.

The supplied temperature table originally labelled the relevant sensor `4F411`; the project owner confirmed that label was incorrect and renamed it `4F412` to match the RDF space. No temperature values changed. In the new scenario, `4F412` measurements are consumed only after simulation for error calculation.
